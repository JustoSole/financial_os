...
export default function PeriodSelector() {
  ...
  const currentValue = dateRange.preset || dateRange.days;
  const currentLabel = PERIOD_OPTIONS.find(opt => opt.value === currentValue)?.label || 'Seleccionar período';

  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const value = Number(e.target.value);
    setDateRange({ preset: value as 7 | 30 | 60 | 90 | 180 | 365 | null });
  };
